...
describe('SelectionDirection', () => {
  ...
  describe('Case Preservation', () => {
    let suggestionPopup: SuggestionPopup;

    beforeEach(() => {
      mockEditor.getLine.mockReturnValue('test line content');
      mockEditor.getCursor.mockReturnValue({ line: 0, ch: 10 });
      suggestionPopup = new SuggestionPopup(mockApp, mockSettings, mockSnippetManager);
      
      // Set up context for suggestion popup
      (suggestionPopup as any).context = {
        start: { line: 0, ch: 8 },
        end: { line: 0, ch: 10 },
        editor: mockEditor
      };
    ...
    it('should preserve lowercase case when selecting suggestion', () => {
      const suggestion = new Suggestion('hello', 'hello', undefined, undefined, {
        originalQueryCase: 'he'
      });
      
      suggestionPopup.selectSuggestion(suggestion, {} as any);
      
      expect(mockEditor.replaceRange).toHaveBeenCalledWith('hello', 
        { line: 0, ch: 8 }, // start
        expect.objectContaining({ line: 0, ch: 10 }) // end
      );
    });

    it('should preserve uppercase case when selecting suggestion', () => {
      const suggestion = new Suggestion('hello', 'hello', undefined, undefined, {
        originalQueryCase: 'HE'
      });
    ...
    it('should preserve capitalized case when selecting suggestion', () => {
      const suggestion = new Suggestion('hello', 'hello', undefined, undefined, {
        originalQueryCase: 'He'
      });
    ...
    it('should use original replacement when no case info', () => {
    ...
    it('should handle snippets with preserved case', () => {
      const suggestion = new Suggestion('hello', 'hello#', undefined, undefined, {
        originalQueryCase: 'He'
      });
      ...
      expect(mockSnippetManager.handleSnippet).toHaveBeenCalledWith('Hello#', 
        { line: 0, ch: 8 }, // start
        mockEditor
      );
    });
